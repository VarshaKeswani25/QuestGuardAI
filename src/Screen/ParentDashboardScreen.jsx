...
import Svg, { Path, Polyline, Line, Polygon } from 'react-native-svg';
...
const PremiumIcon = ({ type, color = T.accent, size = 40 }) => {
  const boxStyle = [
    styles.iconBox,
    { width: size, height: size, borderRadius: size / 2, backgroundColor: color + '15', borderColor: color }
  ];
  ...
  if (type === 'xp') {
    return (
      <View style={boxStyle}>
        <Text style={[styles.premiumIconChar, { color: color, fontSize: 16 }]}>⚡</Text>
      </View>
    );
  }
  if (type === 'mission') {
...
export default function ParentDashboardScreen({ navigation }) {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
            ...
            <TouchableOpacity style={styles.notifBtn} onPress={() => navigation.push('Notifications')}>
              ...
              {pendingMissions.length > 0 && (
                <View style={styles.notifBadge}>
                  <Text style={styles.notifBadgeText}>{pendingMissions.length}</Text>
                </View>
              )}
            </TouchableOpacity>
      ...
      {/* ── INTEGRATED PARENT NAVIGATION BAR ──────────────────────────────── */}
      <View style={styles.navBar}>
        {[
          {
            l: 'Home', sc: 'ParentDashboard',
            icon: (active) => (
          ...
          {
            l: 'Approvals', sc: 'ParentQuestApproval',
            icon: (active) => (
              <Svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                ...
                <Path d="M9 13l2 2 4-4"
                  stroke={active ? theme.accent : theme.textMut} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              </Svg>
            )
          },
          {
          ...
          {
            l: 'Tracking', sc: 'ParentQuestTrack',
            icon: (active) => (
              <Svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                <Line x1="18" y1="20" x2="18" y2="10" stroke={active ? theme.accent : theme.textMut} strokeWidth="2" strokeLinecap="round"/>
                <Line x1="12" y1="20" x2="12" y2="4" stroke={active ? theme.accent : theme.textMut} strokeWidth="2" strokeLinecap="round"/>
                <Line x1="6" y1="20" x2="6" y2="14" stroke={active ? theme.accent : theme.textMut} strokeWidth="2" strokeLinecap="round"/>
              </Svg>
            )
          },
        ].map(n => {
          const active = n.sc === 'ParentDashboard';
          return (
            <TouchableOpacity key={n.l} style={styles.navItem} onPress={() => navigation.navigate(n.sc)}>
              <View style={[styles.navIconWrap, active && { backgroundColor: theme.accent + '20' }]}>
                {n.icon(active)}
              </View>
              <Text style={[styles.navLabel, { color: theme.textMut }, active && { color: theme.accent, fontWeight: '800' }]}>{n.l}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
...
const styles = StyleSheet.create({
  ...
  notifBtn: { width: 40, height: 40, backgroundColor: T.white, borderWidth: 1.5, borderColor: T.border, borderRadius: 20, alignItems: 'center', justifyContent: 'center', position: 'relative' },
