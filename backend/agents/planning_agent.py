...
def get_location_context(location_name):
    try:
        ...
        if not data:
            return {"found": False, "display": location_name, "area_type": "residential area", "description": "residential area of Karachi"}
        result   = data[0]
...
def get_karachi_weather():
    try:
        ...
        return {"condition": "unknown", "temp_c": None, "humidity": None, "is_raining": False, "is_indoor": False, "indoor_reason": ""}
...
def generate_indoor_missions(child_age, reason):
    # ✅ FIX: RAND indoor mein bhi hai ab
    RAND = random.randint(1000, 9999)

    prompt = f"""[Session:{RAND}] You are QuestGuard AI Planning Agent for Karachi.
...
- STRICT: Session {RAND} — generate COMPLETELY DIFFERENT indoor mission titles every time. Never repeat titles from previous sessions.
...
def generate_missions(location, child_age):
    # ✅ FIX: RAND outdoor prompt mein bhi inject ho raha hai
    RAND = random.randint(1000, 9999)
    ...
    if weather["is_indoor"]:
        ...
        indoor_context = [{
            "text":  f"Indoor mode activated due to: {weather['indoor_reason']}. All missions are home-based and safe for a {child_age}-year-old.",
            "page":  "indoor",
            "score": 1.0
        }]
        return {
            "missions":      missions,
            "context_used":  indoor_context,
            "location":      location,
    ...
    prompt = f"""[Session:{RAND}] You are the QuestGuard AI Planning Agent for Karachi.
...
- STRICT: Session {RAND} — generate COMPLETELY DIFFERENT mission titles every single call. Never repeat titles from any previous session. Be creative and vary the civic issues.
